...
### 第一步：讀取 QA Issue + Feature 檔案 + Contracts
...
# 【重要】讀取 contracts — step definitions 必須引用，不准 hardcode
...
cat web/src/contracts.ts        # 共用常數，step impl 應 import
```

**Contract-First 原則（QA 一定遵守）**：
- step definition 內所有 `page.locator('[data-testid="..."]')` 的 testid 必須來自 `contracts.ts.TESTIDS`
- 所有 `expect(...).toContainText('...')` 的文字必須來自 `contracts.ts.TOAST/LABEL`
- 所有 API mock / inject endpoint 必須用 `contracts.ts.API_PATHS`
- **禁止**：在 step impl 自己 hardcode `'sent-record-card'`、`'已送出'`、`/api/debug/inject-ws-event` 這種字串
- 如果 contract 沒涵蓋你需要的：去找 tech-lead / engineer 補 contract，再寫 step
- **import 範例**：
  ```ts
  import { TESTIDS, TOAST, API_PATHS } from '../../web/src/contracts';
  page.locator(`[data-testid="${TESTIDS.sentRecordCard}"]`)
  await expect(toast).toContainText(TOAST.approveSent)
  await request.post(`${BASE_URL}${API_PATHS.injectWsEvent}`, {...})
  ```
