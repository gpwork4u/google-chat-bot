...
| 模組 | 路徑 | 狀態 | 說明 |
|------|------|------|------|
| OAuth flow | `internal/oauth/` | ✅ 完成 | Google OAuth 2.0，AES-GCM 加密 token at rest，HMAC state cookie |
...
| Draft sender | `internal/worker/draft_sender.go` | ✅ 完成 | 透過 extension proxy 走 batchexecute 送出（不需 session.json）|
...
| Auth | Google OAuth 2.0 + extension localhost-only | extension 不認證（單機單使用者） |
